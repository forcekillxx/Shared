...
{
    CCDictionary *levelDic = dynamic_cast<CCDictionary*>(levelInfoDict->objectForKey(CCString::createWithFormat("%s%d",LEVEL_KEY_PREFIX,1)->getCString()));
    CCString *targetScoreString = (CCString*)levelDic->objectForKey("targetScore");
...
{
    CCDictionary *levelDic = dynamic_cast<CCDictionary*>(levelInfoDict->objectForKey(CCString::createWithFormat("%s%d",LEVEL_KEY_PREFIX, 1)->getCString()));
    CCString *moveString = (CCString*)levelDic->objectForKey("moves");
...
void GameState::updateScoreToServer()
{
    ...
    for(int i=1 ; i<=TOTAL_LEVELS ; i++){
        ...
        const char *key = CCString::createWithFormat("%s%d",LEVEL_KEY_PREFIX,i)->getCString();
...
bool GameState::addLevelProgress(int score, int level){
    const char *key = CCString::createWithFormat("%s%d",LEVEL_KEY_PREFIX,level)->getCString();
...
int GameState::getLevelHighScore(int levelNum){
    CCString *key = CCString::createWithFormat("%s%d",LEVEL_KEY_PREFIX,levelNum);
    int levelHighScore = CCUserDefault::sharedUserDefault()->getIntegerForKey(key->getCString());
    return levelHighScore;
}

void GameState::timerForLifeCheck()
{
    struct cc_timeval now;
    CCTime::gettimeofdayCocos2d(&now, NULL);
